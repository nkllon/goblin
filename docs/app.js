...
function renderGraph(container, nodes, edges) {
  ...
  const link = zoomLayer.append("g").attr("stroke", "#999").attr("stroke-opacity", 0.6).selectAll("line").data(edges).join("line").attr("stroke-dasharray", (d) => d.type === "overlapsWith" ? "4,3" : d.type === "dependsOn" ? "2,3" : "0");
  const node = zoomLayer.append("g").selectAll("g").data(nodes).join("g").call(
  ...
  node.append("rect").attr("rx", 8).attr("ry", 8).attr("x", -60).attr("y", -16).attr("width", 120).attr("height", 32).attr("fill", (d) => scoreColor(d.score)).attr("stroke", "#333");
  node.append("text").attr("text-anchor", "middle").attr("dominant-baseline", "middle").attr("font-family", "Helvetica, Arial, sans-serif").attr("font-size", 11).text((d) => d.label);
  const idToNode = {};
  nodes.forEach((n) => {
    idToNode[n.id] = n;
  });
  ...
  svg.call(zoom_default2().scaleExtent([0.3, 3]).on("zoom", (event) => zoomLayer.attr("transform", event.transform)));
}
async function main() {
